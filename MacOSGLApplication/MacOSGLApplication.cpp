...
namespace T3D 
{
	...
	MacOSGLApplication::MacOSGLApplication(void)
	{
		window = NULL;
		running = false;
...
//		}
	...
	int MacOSGLApplication::run(void){
		if(init() == false) {
			return -1;
		}
		running = true;
		...
		while(running) {
            ...
            //MACOS DIFFERENT: THIS WAS ORIGINALLY DONE IN renderer->postrender();
	...
	void MacOSGLApplication::quit(void){
		SDL_Quit();
	}

	void MacOSGLApplication::handleEvent(SDL_Event *e){
		switch (e->type) {
			case SDL_QUIT:
				running = false;
				break;
