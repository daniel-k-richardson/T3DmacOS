...
namespace T3D{
	...
	bool T3DTest::init(){
		...
		renderer->loadSkybox(getResourcePath("Sunny1"));
		...
		Texture *smileytex = new Texture(getResourcePath("Smiley.png"), true, true);
		renderer->loadTexture(smileytex);
		Material *smiley = renderer->createMaterial(Renderer::PR_OPAQUE);
		smiley->setTexture(smileytex);
		
		Texture *sparkletex = new Texture(getResourcePath("sparkle.png"), true, true);
		...
		font *f = getFont("/Users/danielrichardson/Documents/Development/Projects/T3D/T3DmacOS/MacOSGLApplication/Resources/FreeSans.ttf", 24);
		if (f != NULL) {
